                ...
                <div class="card">
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                        <p class="card-title my-0">{{ $pageTitle ?? 'Page Title' }} <span class="badge bg-danger side-badge"
                                style="font-size:17px;">{{ count($courses) }}</span> </p>
                                ...
                                <tbody>
                                    @foreach ($courses as $key => $course)
                                        <tr>
                                            ...
                                            <td>{{ $course->category->name_en ?? 'N/A' }}</td>
                                            ...
                                            <td>
                                                @if ($course->discount_price > 0)
                                                    @if ($course->discount_type == 1)
                                                        <span
                                                            class="badge rounded-pill bg-info text-white">৳{{ $course->discount_price }}
                                                            off</span>
                                                    @elseif($course->discount_type == 2)
                                                        <span
                                                            class="badge rounded-pill bg-success text-white">{{ $course->discount_price }}%
                                                            off</span>
                                                    @endif
                                                @else
